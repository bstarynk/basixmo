...
## Dont change the name basixmo-dump-state.sh of this script without
## care, it appears elsewhere (in Makefile & in basixmo.h)
...
if file "$dbfile" | grep -qi SQLite ; then
    echo "$0:" dumping Basixmo Sqlite database $dbfile
else
...
echo " --  This sqlite3 dump file $sqlbase is part of BASIXMO." >> $tempdump
echo ' --' >> $tempdump
echo ' --  BASIXMO is free software; you can redistribute it and/or modify' >> $tempdump
...
echo ' --' >> $tempdump
echo ' --  BASIXMO is distributed in the hope that it will be useful,' >> $tempdump
...
echo ' --  along with BASIXMO; see the file COPYING3.   If not see' >> $tempdump
